...
const ROUTE_LOADERS = {
  about: () => import('./pages/about').then((module) => module?.AboutPage ?? null),
  ban: () => import('./pages/ban').then((module) => module?.BanPage ?? null),
  blog: () => import('./pages/blog').then((module) => module?.BlogPage ?? null),
  'blog-submit': () =>
    import('./pages/blog-submit').then((module) => module?.BlogSubmissionPage ?? null),
  classements: () =>
    import('./pages/classements').then((module) => module?.ClassementsPage ?? null),
  cgu: () => import('./pages/cgu').then((module) => module?.CguPage ?? null),
  'cgv-vente': () =>
    import('./pages/cgv-vente').then((module) => module?.CgvVentePage ?? null),
  chat: () => import('./pages/chat').then((module) => module?.ChatPage ?? null),
  'mentions-legales': () =>
    import('./pages/mentions-legales').then((module) => module?.MentionsLegalesPage ?? null),
  salons: () => import('./pages/salons').then((module) => module?.SalonsPage ?? null),
  members: () => import('./pages/members').then((module) => module?.MembersPage ?? null),
  profile: () => import('./pages/profile').then((module) => module?.ProfilePage ?? null),
  shop: () => import('./pages/shop').then((module) => module?.ShopPage ?? null),
  statistiques: () =>
    import('./pages/statistiques').then((module) => module?.StatistiquesPage ?? null),
};
...
const buildBootstrapState = (rawState) => {
  ...
  const pages = rawState.pages && typeof rawState.pages === 'object' ? rawState.pages : {};

  return {
    route: cloneRouteDescriptor(rawState.route),
    participants: normalizeBootstrapParticipants(rawState.participants),
    listenerStats: normalizeBootstrapListenerStats(rawState.listenerStats),
    bridgeStatus: normalizeBootstrapBridgeStatus(rawState.bridgeStatus),
    pages,
    pulse: normalizePulsePresentation(pages?.home?.pulse ?? null),
  };
};
...
const AppShell = ({ initialUrl = null, bootstrapState = null } = {}) => {
  const bootstrap = useMemo(() => buildBootstrapState(bootstrapState), [bootstrapState]);
  const bootstrapRoute = bootstrap.route;
  const bootstrapParticipants = bootstrap.participants;
  const bootstrapListenerStats = bootstrap.listenerStats;
  const bootstrapPulse = bootstrap.pulse;
  const bootstrapBridgeStatus = bootstrap.bridgeStatus;
  const bootstrapPages = bootstrap.pages;

  const initialRouteDescriptor = useMemo(() => {
    if (bootstrapRoute) {
      return bootstrapRoute;
    }
    if (initialUrl) {
      try {
        const url = new URL(initialUrl, 'https://libre-antenne.local');
        return parseRouteFromLocation({ pathname: url.pathname, search: url.search });
      } catch (error) {
        console.warn('Impossible de déterminer la route initiale', error);
      }
    }
    return initializeRoute();
  }, [bootstrapRoute, initialUrl]);

  const [status, setStatus] = useState('connecting');
  const [participantsMap, setParticipantsMap] = useState(() => {
    const map = new Map();
    for (const participant of bootstrapParticipants) {
  ...
  const [route, setRoute] = useState(() => initialRouteDescriptor);
  ...
  const [listenerStats, setListenerStats] = useState(() => (
    bootstrapListenerStats
      ? { count: bootstrapListenerStats.count, history: bootstrapListenerStats.history.slice() }
      : { count: 0, history: [] }
  ));
  const [communityPulse, setCommunityPulse] = useState(() => bootstrapPulse);
  const [guildSummary, setGuildSummary] = useState(null);
  const [bridgeStatus, setBridgeStatus] = useState(() =>
    bootstrapBridgeStatus
      ? { ...bootstrapBridgeStatus }
  ...
  useEffect(() => {
    ...
    const connection =
      navigatorAny.connection || navigatorAny.mozConnection || navigatorAny.webkitConnection || null;
  ...
  const navigateToRoute = useCallback(
    (
      ...
      }: { replace?: boolean; scrollToTop?: boolean; behavior?: ScrollBehavior } = {},
    ) => {
      prefetchRoute(name);
  ...
  const updateProfileRoute = useCallback(
    (
      ...
      sinceMs,
      untilMs,
      options: { replace?: boolean; scrollToTop?: boolean; behavior?: ScrollBehavior } = {},
    ) => {
      if (!userId) {
        return;
      }
          ...
          ${
            route.name === 'cgu'
              ...
              ? renderAsyncPage('blog', {
                  params: route.params,
                  bootstrap: bootstrapPages.blog ?? null,
              ...
              : route.name === 'shop'
              ? renderAsyncPage('shop', { bootstrap: bootstrapPages.shop ?? null })
              : route.name === 'profile'
              ...
              ? renderAsyncPage('statistiques', {
                  params: route.params,
                  bootstrap: bootstrapPages.statistiques ?? null,
                  onSyncRoute: (nextParams, options: { scrollToTop?: boolean } = {}) =>
              ...
              ? renderAsyncPage('classements', {
                  params: route.params,
                  bootstrap: bootstrapPages.classements ?? null,
                  onSyncRoute: (nextParams, options: { scrollToTop?: boolean } = {}) =>
              ...
              : route.name === 'salons'
              ? renderAsyncPage('salons', { bootstrap: bootstrapPages.salons ?? null })
